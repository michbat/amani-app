...
@section('title', 'Ajout de tags au menu')
...
@section('content')
    ...
    <div class="card mt-5">
        <div class="card-header">
            <h4>Les tags associés à ce menu: </h4>
            @if ($menu->tags->count() > 0)
                @foreach ($menu->tags as $tag)
                    <form action="{{ route('admin.menus.tags.remove', [$menu->id, $tag->id]) }}" method="POST">
                        @csrf
                        @method('delete')
                        <button type="submit"
        ...
        <form action="{{ route('admin.menus.tags', $menu->id) }}" method="POST">
            @csrf
            <div class="card-body m-3">
                <div class="form-group" id="tata">
                    ...
                    <select name="tags[]" class="custom-select" multiple  size="20">
                        @foreach ($tags as $tag)
                            <option value="{{ $tag->id }}" @selected($menu->hasTag($tag->name))>
                                {{ $tag->name }}
                            </option>
                        @endforeach
                    </select>
