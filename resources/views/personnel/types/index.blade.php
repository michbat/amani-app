...
@section('content')
    <div class="d-flex mt-5 justify-content-end">
        <a class="btn btn-info btn-lg text-dark" href="{{ route('personnel.types.create') }}"><i
                class="fas fa-plus mx-2"></i>Ajouter un type d'ingrédients</a>
    </div>
    <div class="card mt-3">
        <div class="card-header d-flex flex-column justify-content-center align-items-start">
            <h4>Liste de type d'ingrédients</h4>
            {{-- <p>{{ $ingredients->count() }} sur {{ $ingredients->total() }}</p> --}}
            {{-- <p>{{ $ingredients->currentPage() }} sur {{ $ingredients->perPage() }}</p> --}}
            <p>{{ $types->firstItem() }} à {{ $types->lastItem() }} sur {{ $types->total() }} type(s)</p>
        </div>
        <div class="card-body">
                                ...
                                <td>
                                    <div class="d-flex justify-content-center">
                                        ...
                                        <a class="btn btn-primary mx-2 text-dark"
                                            href="{{ route('personnel.types.edit', $type->id) }}"><i
                                                class="fas fa-edit mx-2"></i>Editer</a>

                                        <form action="{{ route('personnel.types.destroy', $type->id) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
